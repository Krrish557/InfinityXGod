...
from tgbot import LOGGER, SUDOS,  dispatcher
...
from tgbot.database.sql import reporting_sql as sql
    ...
    if chat.type == chat.PRIVATE:
        if len(args) >= 1:
            if args[0] in ("yes", "on"):
                ...
                msg.reply_text(
                    "Reports turned ON! you will be notified on any incoming report.",
                )

            elif args[0] in ("no", "off"):
                sql.set_user_setting(chat.id, False)
                msg.reply_text("Turned off reporting! You will not be recieving any reports now.")
        else:
            msg.reply_text(
...
def report(update: Update, context: CallbackContext) -> str:
    ...
    if chat and message.reply_to_message and sql.chat_should_report(chat.id):
        ...
        if reported_user.id in REPORT_IMMUNE_USERS:
            message.reply_text("Uh? You reporting a member of Kingdom Assosiation?")
            return ""
