...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text("Could not find this this user.")
        return log_message
    if user_id == bot.id:
        message.reply_text("I would prefer not being banned my self! :)")
        return log_message

    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("I can't ban my own Owner.")
        elif user_id in DEV_USERS:
            message.reply_text("He is my DEV i can not ban him.")
        elif user_id in SUDOS:
            message.reply_text(
                "Fighting Gen will put villager's lives at risk."
            )
        elif user_id in HELPERS:
            message.reply_text(
                "Bring an order from Captain to fight a Assasin servant."
            )
        
        else:
            message.reply_text("⚠️ Cannot ban admins.")
        return log_message
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            message.reply_text("Some exception occured")
...
def temp_ban(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        if excp.message != "User not found":
            raise
        message.reply_text("User could not be found.")
        return log_message
    if user_id == bot.id:
        message.reply_text("I would prefer not being banned my self! :)")
        return log_message
    ...
    if not reason:
        message.reply_text("You have not specified a time to ban this user for!")
        return log_message
    ...
    try:
        ...
        if excp.message == "Replied message not found":
            ...
            message.reply_text("The user can not be banned.")
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("The user is protected!")
        return log_message
...
def selfunban(update: Update, context: CallbackContext) -> str:
    ...
    user = update.effective_user
    bot, args = context.bot, context.args
    if user.id not in SUDOS  or user.id not in DEV_USERS:
        return
