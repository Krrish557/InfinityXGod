...
from tgbot import dispatcher, SUDOS
...
import tgbot.database.sql.approve_sql as sql
...
def approve(update, context):
    ...
    if member.status in ("administrator", "creator"):
        message.reply_text(
            "Specified user is already Admin",
        )
        return ""
    ...
    message.reply_text(
        f"[{member.user['first_name']}](tg://user?id={member.user['id']}) is now approved in {chat_title}! They will now be ignored by automated admin actions like locks, blocklists, and antiflood.",
        parse_mode=ParseMode.MARKDOWN,
    )
    log_message = (
...
def disapprove(update, context):
    ...
    try:
        ...
        return ""
    if member.status in ("administrator", "creator"):
        message.reply_text("Specified user is already Admin.")
        return ""
    if not sql.is_approved(message.chat_id, user_id):
        message.reply_text(f"{member.user['first_name']} is not approved yet!")
        return ""
...
def approved(update, context):
    ...
    if msg.endswith("approved.\n"):
        message.reply_text(f"No one is approved in {chat_title}.")
        return ""
