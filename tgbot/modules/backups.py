...
from telegram import ParseMode, Message, User
from telegram.error import BadRequest
from telegram.ext import CommandHandler

import tgbot.database.sql.notes_sql as sql
from tgbot import dispatcher, LOGGER, OWNER_ID
from tgbot.vars import LOG_CHANNEL, SUPPORT_CHAT
from tgbot import DATA_IMPORT
...
import tgbot.database.sql.rules_sql as rulessql

# from tgbot.database.sql import warns_sql as warnssql
import tgbot.database.sql.blacklist_sql as blacklistsql
from tgbot.database.sql import disable_sql as disabledsql

# from tgbot.database.sql import cust_filters_sql as filtersql
# import tgbot.database.sql.welcome_sql as welcsql
import tgbot.database.sql.locks_sql as locksql
...
def import_data(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text("This command can not be used in private chat!")
            return ""
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if len(data) > 1 and str(chat.id) not in data:
            msg.reply_text(
                "There are more than one group in this file and the chat.id is not same! I can not import this!",
            )
            return
        ...
        try:
            if str(context.bot.id) != str(data[str(chat.id)]["bot"]):
                return msg.reply_text(
                    "Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audio and records might not work as it should be.",
                )
        except Exception:
            pass
        # Select data source
    ...
    if checkchat.get("status"):
        if jam <= int(checkchat.get("value")):
            ...
            update.effective_message.reply_text(
                "BackUp can be made only once a day\nWait for `{}` to create a BackUp again.".format(
                    timeformatt,
                ),
