...
from tgbot.database.sql import antiflood_sql as sql
...
from tgbot.database.sql.approve_sql import is_approved
...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        msg.reply_text(
            "Anti-flood have been disabled because of insufficient permissions given to me.",
        )
...
def set_flood(update, context) -> str:
    ...
    if len(args) >= 1:
        ...
        if val in ["off", "no", "0"]:
            ...
            if conn:
                message.reply_text(
                    "Antiflood is disabled in {}.".format(chat_name),
                )
            else:
                message.reply_text("Antiflood is disabled.")
            ...
            if amount <= 0:
                ...
                if conn:
                    message.reply_text(
                        "Antiflood is disabled now in {}.".format(chat_name),
                    )
                else:
                    message.reply_text("Antiflood is disabled now.")
                return (
                    "<b>{}:</b>"
                    "\n#SETFLOOD"
                    "\n<b>Admin:</b> {}"
                ...
                if conn:
                    message.reply_text(
                        "Anti-flood is set to {} in chat: {}".format(
                            amount, chat_name,
                        ),
                    )
                else:
                    message.reply_text(
                        "Anti-flood limit updated to {}!".format(amount),
                    )
                return (
                    "<b>{}:</b>"
                    "\n#SETFLOOD"
...
def flood(update, context):
    ...
    if limit == 0:
        if conn:
            msg.reply_text(
                "Flood Control is not being enforced in {}!".format(chat_name),
            )
        else:
            msg.reply_text("Flood Control is not being enforced in this group!")
    else:
        if conn:
            msg.reply_text(
                "Everyone who sends {} consecutive messages will be restricted in {}.".format(
                    limit, chat_name,
                ),
            )
        else:
            msg.reply_text(
                "Everyone who sends {} consecutive messages will be restricted.".format(
                    limit,
                ),
            )
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks.""",
        ...
        if conn:
            msg.reply_text(
                "Do not spam, or else you will get {} in {}.".format(
                    settypeflood, chat_name,
                ),
            )
        else:
            msg.reply_text(
                "Do not spam, or else you will get {}.".format(
                    settypeflood,
                ),
            )
    return ""
