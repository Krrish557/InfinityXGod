...
from tgbot import TOKEN, USER_INFO
import tgbot.database.sql.userinfo_sql as sql
...
from tgbot.database.redis.afk_redis import is_user_afk, afk_reason
from tgbot.database.sql.users_sql import get_user_num_chats
...
@telethn.on(
    events.NewMessage(
        pattern="/ginfo ", from_users=(SUDOS or []) + (HELPERS or []),
    ),
)
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        message.reply_text("User could not be extracted from this.")
        return
...
def about_me(update: Update, context: CallbackContext):
    ...
    if info:
        ...
        update.effective_message.reply_text(
            f"{username} haven't set an info message about themselves yet!",
        )
    else:
...
def about_bio(update: Update, context: CallbackContext):
    ...
    if info:
        ...
        update.effective_message.reply_text(
            f"{username} havn't had a message set about themselves yet!\nSet one using /setbio",
        )
    else:
        update.effective_message.reply_text(
            "You do not have a bio set about yourself yet!",
        )
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "uhm.. yeeah, I only trust my best friends to set my bio.",
            )
            return
