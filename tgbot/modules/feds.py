...
import tgbot.database.sql.feds_sql as sql
from tgbot import (
    LOGGER,
    OWNER_ID,
    SUDOS,
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                f"Can't federate! Please contact admins in @{SUPPORT_CHAT} if the problem persist.",
            )
            return

        update.effective_message.reply_text(
            "*You New federation created!*"
            "\nName: `{}`"
            "\nID: `{}`"
...
def fed_chat(update: Update, context: CallbackContext):
    ...
    text = "This group is part of the subsequent federation:"
...
def join_fed(update: Update, context: CallbackContext):
        ...
        for admin in administrators:
            ...
            if status == "creator":
                if str(admin.user.id) == str(user.id):
                    ...
                    update.effective_message.reply_text(
                        "Only chat creators can use this command!",
                    )
                    return
    if fed_id:
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                f"Failed to join federation! Please contact admins in @{SUPPORT_CHAT} should this problem persist!",
            )
            return
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in SUDOS:
        ...
        if user_id == get_owner:
            update.effective_message.reply_text(
                "This is user owner of the federation",
            )
            return
        if getuser:
            update.effective_message.reply_text(
