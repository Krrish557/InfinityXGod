...
import time
from tgbot.database.redis import REDIS
...
from tgbot.database.redis.afk_redis import start_afk, end_afk, is_user_afk, afk_reason
...
def afk(update, context):
    ...
    try:
        Shikimori = update.effective_message.reply_text(
            "*{}* is now away! We won't miss you! :)".format(fname), parse_mode=ParseMode.MARKDOWN)
        time.sleep(5)
        try:
            Shikimori.delete()
        except BadRequest:
    ...
    if res:
        ...
        try:
             text = message.reply_text(
                "*{}* is back after: `{}`".format(firstname, end_afk_time), parse_mode=ParseMode.MARKDOWN)
             time.sleep(5)
             try:
                 text.delete()
             except BadRequest:
