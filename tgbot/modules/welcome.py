...
import tgbot.database.sql.welcome_sql as sql
from tgbot import (
    DEV_USERS,
    OWNER_ID,
    SUDOS,
...
from tgbot.database.sql.global_bans_sql import is_user_gbanned
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                try:
                    ...
                    update.effective_message.reply_text("Hold Your breath!! My Owner have stepped in.", reply_to_message_id=reply)
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "I will miss you :(", reply_to_message_id=reply
                )
                return
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                "Okay! I will greet members when they join."
            )

        elif args[0].lower() in ("off", "no"):
            sql.set_welc_preference(str(chat.id), False)
            update.effective_message.reply_text(
                "I will go loaf around and not welcome anyone from now on."
            )

        else:
            update.effective_message.reply_text(
                "Please reply 'on/yes' or 'off/no'!"
            )
...
def set_welcome(update: Update, context: CallbackContext) -> str:
    ...
    if data_type is None:
        msg.reply_text("You have not specify what to reply with!")
        return ""
...
def reset_goodbye(update: Update, context: CallbackContext) -> str:
    ...
    update.effective_message.reply_text(
        "Reset message have been reset to default!"
    )
...
def clean_welcome(update: Update, context: CallbackContext) -> str:
    ...
    if args[0].lower() in ("off", "no"):
        ...
        update.effective_message.reply_text("I will not delete old welcome messages.")
        return (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#CLEAN_WELCOME\n"
            f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
    ...
    update.effective_message.reply_text("Please reply 'on/yes' or 'off/no'!")
    return ""
