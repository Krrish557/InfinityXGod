...
from tgbot.database.sql import warns_sql as sql
...
def warn_user(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        message.reply_text("Please provde a valid UserID.")
    return ""
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            ...
            update.effective_message.reply_text(
                f"User has {num_warns}/{limit} warns, but no reasons for any of them. XD"
            )
    else:
        update.effective_message.reply_text("This user do not have any warns!")
...
def remove_warn_filter(update: Update, context: CallbackContext):
    ...
    msg.reply_text(
        "That's not a warning filter\nUse /warnlist for all active warning filters."
    )
...
def set_warn_limit(update: Update, context: CallbackContext) -> str:
    ...
    if args:
        if args[0].isdigit():
            ...
            msg.reply_text("Give me a number as an input!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
...
def set_warn_strength(update: Update, context: CallbackContext):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text("Too many warns will consequently give you a Ban!")
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
