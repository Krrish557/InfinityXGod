...
from telegram import Bot, CallbackQuery, ParseMode, Update, InlineKeyboardButton, InlineKeyboardMarkup
...
from tgbot import SUDOS, dispatcher,DEV_USERS, telethn as bot
...
def set_sticker(update: Update, context: CallbackContext):
    ...
    if user_can_changeinfo(chat, user, context.bot.id) is False:
        return msg.reply_text("Apologies for the oversight. You need to have the appropriate permissions to change chat information.")

    if msg.reply_to_message:
        if not msg.reply_to_message.sticker:
            return msg.reply_text(
                "To set the chat sticker set, you need to reply to a sticker message!"
            )
        stkr = msg.reply_to_message.sticker.set_name
        try:
            context.bot.set_chat_sticker_set(chat.id, stkr)
            msg.reply_text(f"The new group stickers in {chat.title} have been set successfully!")
        except BadRequest as excp:
            if excp.message == "Participants_too_few":
                return msg.reply_text(
                    "Apologies for the inconvenience. Unfortunately, according to Telegram restrictions, a chat must have a minimum of 100 members before group stickers can be set. Please make sure the chat has reached the required number of members before attempting to set group stickers."
                )
...
def setchatpic(update: Update, context: CallbackContext):
    ...
    if user_can_changeinfo(chat, user, context.bot.id) is False:
        msg.reply_text("Apologies for the oversight. You need to have the appropriate permissions to change chat information!")
        return
    ...
    if msg.reply_to_message:
        if msg.reply_to_message.photo:
            ...
            msg.reply_text("Only photos can be set as the chat picture, not videos or documents.")
            return
        dlmsg = msg.reply_text("🔎")
        tpic = context.bot.get_file(pic_id)
        tpic.download("gpic.png")
        try:
            with open("gpic.png", "rb") as chatp:
                context.bot.set_chat_photo(int(chat.id), photo=chatp)
                msg.reply_text("Successfully set new Group Profile Pics!")
        except BadRequest as excp:
            msg.reply_text(f"Error! {excp.message}")
        finally:
            dlmsg.delete()
...
def rmchatpic(update: Update, context: CallbackContext):
    ...
    try:
        ...
        msg.reply_text("Successful deletion chat's profile photo!")
    except BadRequest as excp:
        msg.reply_text(f"Error! {excp.message}.")
        return
...
def set_desc(update: Update, context: CallbackContext):
    ...
    if len(tesc) >= 2:
        ...
        return msg.reply_text("Please provide valid Description")
    try:
        if len(desc) > 255:
            return msg.reply_text("Description must be under 255 characters.!")
        context.bot.set_chat_description(chat.id, desc)
        msg.reply_text(f"Chat description updates successfully {chat.title}!")
    except BadRequest as excp:
        msg.reply_text(f"Error! {excp.message}.")        
...
def setchat_title(update: Update, context: CallbackContext):
    ...
    title = " ".join(args)
    if not title:
        msg.reply_text("Give a valid Text")
        return
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if (
        ...
        message.reply_text("Apologies for the oversight. You need to have the appropriate permissions to change chat information!")
        return
    ...
    if not user_id:
        message.reply_text(
            "Either you have not reffered to a user or ID provided is incorrect.",
        )
        return
    ...
    if user_member.status in ('administrator', 'creator'):
        message.reply_text("The user you are trying to promote, is already an admin.")
        return

    if user_id == bot.id:
        message.reply_text("I can not promote myself. Please do it manually through edit group settings.")
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text("User is not present in the group.")
        else:
    ...
    bot.sendMessage(
        chat.id,
        f'''Promoting {user_member.user.username} in <b>{chat.title}</b>:
