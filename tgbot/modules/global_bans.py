...
import tgbot.database.sql.global_bans_sql as sql
from tgbot.database.sql.users_sql import get_user_com_chats
from tgbot import (
    DEV_USERS,
    OWNER_ID,
    SUDOS,
...
from tgbot.vars import SUPPORT_CHAT, LOG_CHANNEL
...
def gban(update: Update, context: CallbackContext):
    ...
    if user_id == bot.id:
        message.reply_text("Can not ban myself!")
        return

    if user_id in [777000, 1087968824]:
        message.reply_text("You can not attack Telegram's native tech!")
        return
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if not reason:
            message.reply_text(
                "This user is already gbanned; I'd change the reason, but you have not given me one...",
            )
            return
        ...
        if old_reason:
            ...
            message.reply_text(
                "This user is already gbanned, but had no reason set; I have updated it!",
            )
    ...
    if LOG_CHANNEL:
        ...
        send_to_list(bot, SUDOS + HELPERS, log_message, html=True)
    ...
    if gban_time > 60:
        ...
        message.reply_text("Done! Globally banned.", parse_mode=ParseMode.HTML)
    else:
        message.reply_text("Done! Globally banned.", parse_mode=ParseMode.HTML)
...
def ungban(update: Update, context: CallbackContext):
    ...
    if LOG_CHANNEL:
        ...
        send_to_list(bot, SUDOS + HELPERS, "un-gban complete!")
