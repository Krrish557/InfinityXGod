...
BASE.metadata.create_all(bind=SESSION.get_bind(), tables=[Users.__table__, Chats.__table__, ChatMembers.__table__])
...
def ensure_bot_in_db():
    with INSERTION_LOCK:
        bot = Users(pbot.id, pbot.username)
        SESSION.merge(bot)
        SESSION.commit()
...
def action():
    global TOTAL_CHATS, TOTAL_USERS
    TOTAL_USERS = num_users()
    TOTAL_CHATS = num_chats()


set_interval(action, 10)
